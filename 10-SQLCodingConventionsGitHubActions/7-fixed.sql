...
FROM INTERNETPACKAGE
WHERE PACKAGEID < ANY(
    SELECT AVG(PRICEPERMONTH) AS AVG_PP
    FROM BILL
...
WITH BILLAMOUNT_AVG AS (
    SELECT
        BILLID,
        AVG(BILLAMOUNT) AS AVG_B_AMOUNT
    FROM BILL GROUP BY BILLID
)
...
FROM CLIENT, BILL, INTERNETPACKAGE, BILLAMOUNT_AVG
WHERE BILLAMOUNT_AVG.BILLID = CLIENT.CLIENTID
    AND BILL.BILLAMOUNT > BILLAMOUNT_AVG.AVG_B_AMOUNT;
...
*/
SELECT INTERNETPACKAGE.PACKAGETITLE
FROM INTERNETPACKAGE
WHERE EXISTS (
    SELECT BILL.BILLID
    FROM BILL
    WHERE BILL.PACKAGEID = INTERNETPACKAGE.PACKAGEID
    );
